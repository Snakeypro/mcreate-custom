...
public class ${getClassName()}Block extends ${getBlockClass(data.blockBase)}
	...
	<#if data.hasInventory>
		<#if isKineticBlock>
			<#assign interfaces += ["IBE<${getClassName()}BlockEntity>"]>
		<#else>
	...
	</#if>
{
	...
	<#if data.rotationMode != 0 && !isKineticBlock>
		<#if data.rotationMode != 5>
	...
	<#if data.hasInventory>
		...
		<#if isKineticBlock>
		@Override
		public Class<${name}BlockEntity> getBlockEntityClass() {
			return ${name}BlockEntity.class;
		}

		@Override
		public BlockEntityType<? extends ${name}BlockEntity> getBlockEntityType() {
			return ${JavaModName}BlockEntities.${REGISTRYNAME}.get();
		}
		<#else>
	    ...
	    <#if data.inventoryComparatorPower && !isKineticBlock>
