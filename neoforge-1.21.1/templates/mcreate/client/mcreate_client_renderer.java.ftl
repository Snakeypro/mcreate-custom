package ${package}.mcreate.client;
...
public class McreateClientRenderer extends KineticBlockEntityRenderer<CustomKineticBlockEntity> {
    ...
    private static final Map<Direction, float[]> COG_ROTATIONS = new EnumMap<>(Map.of(
        Direction.NORTH, new float[]{90f,   0f,   0f},
        Direction.SOUTH, new float[]{90f, 180f,   0f},
        Direction.EAST,  new float[]{90f,   0f,  90f},
        Direction.WEST,  new float[]{90f, 180f,  90f},
        Direction.UP,    new float[]{ 0f,   0f,   0f},
        Direction.DOWN,  new float[]{ 0f,   0f,   0f}
    ));

    @Override
    protected void renderSafe(CustomKineticBlockEntity be, float partialTicks, PoseStack ms,
